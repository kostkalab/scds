...
We can visualize the doublets after projecting into two dimensions:
...
We now run the ```scds``` doublet annotation appraoches ([see manuscript](https://doi.org/10.1101/564021)). We focus on ```cxds```, because ```bcds``` appears to work better for data sets with more cells.
...
```{r pairplot, eval = TRUE, echo = TRUE, message = FALSE, warning = FALSE}
